...
BaseWidget.prototype.hasBindable = function (bindableName) {
  return this.getBindables().hasOwnProperty(bindableName);
}

BaseWidget.prototype.getBindables = function () {
  var self = this;

  if (self.bindables) {
    return self.bindables;
  }
...
BaseWidget.prototype.declareBindables = function (newBindables) {
  var self = this;

  _.forOwn(newBindables, function (value, key) {
  ...
  self.bindables = _.merge(self.bindables || {}, newBindables);
  return self;
};

BaseWidget.prototype.executeBindable = function (bindableName, args) {
  var self = this;

  if ( self.bindables[bindableName].action ) {
    return self.bindables[bindableName].action.apply(self, args);
  }

  throw new Error ('Bindable does not exist');
  return self;
};
